...
res_base_path = "materials/Risorse_lessicali/"
tweets_path = "materials/Twitter_messaggi/"
#feeling_list = ['Fear']
...
def create_twitter_mongo(feeling):
    ...
    try:
        ...
        try:
            ...
            print(write_errors)
        print("Sharding collection created...")
...
def get_stats():
    out_handle = open('out/stats.txt', 'w')
...
def convertDictToList(feeling_toConv):
    final_list = []
    for key, value in tweets[feeling_toConv].items():
        final_list.append(key)
    for key, value in emoji[feeling_toConv].items():
        final_list.append(key)
    for key, value in tags[feeling_toConv].items():
        final_list.append(key)
    return final_list


def printHistogram(feeling_selected):
    list_shared = {}
    histogram_values = []
    labels = []
    current_feeling = convertDictToList(feeling_selected)
    for feel in feeling_list:
        if feel != feeling_selected:
            labels.append(feel)
            other_feeling = convertDictToList(feel)
            current_feeling_set = set(current_feeling)
            list_shared = list(current_feeling_set.intersection(other_feeling))
            print("Feeling: " + feeling_selected + "- Feel: " + feel + " " + str(len(list_shared)))
            print("TOTALE: " + str(len(list_shared) / len(current_feeling)))
            # preparo i valori
            histogram_values.append(len(list_shared) / len(current_feeling))

    # plot histogram
    histogram_values_series = pd.Series(histogram_values)
    plt.figure(figsize=(12, 8))
    ax = histogram_values_series.plot(kind="bar")
    ax.set_title('perc_presence_twitter(x, ' + feeling_selected + ')')
    ax.set_xlabel("Amount")
    ax.set_ylabel("Percentage")
    ax.set_xticklabels(labels)
    plt.savefig('histograms/perc_presence_twitter(x, ' + feeling_selected + ').png')

def printTopFive(feeling_selected):
    #emoji
    sorted_emoji = dict(sorted(emoji[feeling_selected].items(), key=operator.itemgetter(1), reverse=True))
    print(list(sorted_emoji.items())[:5])
    print("Valori totali emoji "+feeling_selected+": "+str(sum(sorted_emoji.values())))
    print("--------------------------------")
    #hashtag
    sorted_hashtag = dict(sorted(tags[feeling_selected].items(), key=operator.itemgetter(1), reverse=True))
    print(list(sorted_hashtag.items())[:5])
    print("Valori totali hashtag " + feeling_selected + ": " + str(sum(sorted_hashtag.values())))
    print("--------------------------------")
    #word
    sorted_tweets = dict(sorted(tweets[feeling_selected].items(), key=operator.itemgetter(1), reverse=True))
    print(list(sorted_tweets.items())[:5])
    print("Valori totali words " + feeling_selected + ": " + str(sum(sorted_tweets.values())))
    print("--------------------------------")

createSharded()
#createNS()
#executeMapReduce()
#postgresAnalysis() Scommentare dentro create_lexical_res()
printStats()
#for feeling in feeling_list:
    #printTopFive(feeling)
    #printHistogram(feeling)